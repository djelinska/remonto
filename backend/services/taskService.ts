...
export const createProjectTask = async (userId: ObjectId, projectId: ObjectId, newTask: TaskData): Promise<Task> => {
	try {
		...
		if (!project) {
			throw new Error("Project not found or you don't have permission to add tasks.");
		}
...
export const updateProjectTask = async (userId: ObjectId, projectId: ObjectId, taskId: ObjectId, updatedTask: TaskData): Promise<Task> => {
	try {
		...
		if (!project) {
			throw new Error("Project not found or you don't have permission to update tasks.");
		}

		const task: TaskDto | null = await TaskModel.findOneAndUpdate({ _id: taskId, projectId }, updatedTask, { new: true, runValidators: true });
...
export const deleteProjectTask = async (userId: ObjectId, projectId: ObjectId, taskId: ObjectId): Promise<ReturnMessage> => {
	try {
		...
		if (!project) {
			throw new Error("Project not found or you don't have permission to delete tasks.");
		}
