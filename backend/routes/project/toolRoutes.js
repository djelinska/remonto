...
const validateToolData = require("../../middlewares/validateTool");
...
router.post("/", [authenticateUser, validateToolData], async (req, res) => {
    try {
        const { projectId } = req.params;
        const userId = req.user.id;
...
router.put("/:toolId", [authenticateUser, validateToolData], async (req, res) => {
    try {
        const { projectId, toolId } = req.params;
        const userId = req.user.id;
